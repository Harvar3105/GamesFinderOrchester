...
import { v4 } from 'uuid';
export async function fetchSteamGame(id) {
    const url = `https://store.steampowered.com/api/appdetails?appids=${id}&cc=us&l=en`;
    const res = await fetch(url);
    const data = await res.json(); //TODO: Type this properly
    if (!data[id]?.success)
        return null;
...
//   return results;
// }
export async function scrapeBatch(ids) {
    const results = [];
    results.push(...(await Promise.all(ids.map(fetchSteamGame)))
        .filter(g => g !== null));
    return results;
}
